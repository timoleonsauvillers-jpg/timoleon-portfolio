...
export function ProductClient({ product }: ProductClientProps) {
  ...
  return (
    <div className="md:fixed md:inset-0 md:pt-nav-height md:pb-footer-height md:flex md:items-center pt-nav-height min-h-screen">
      {/* Desktop layout */}
      ...
      {/* Mobile layout - scrollable, image then info */}
      <div className="flex md:hidden w-full flex-col">

        {/* Flèche retour fixe en haut à gauche */}
        <Link
          href="/shop"
          className="fixed top-nav-height left-4 z-20 text-nav text-muted hover:text-foreground transition-colors duration-300 py-3"
        >
          ← Retour
        </Link>

        {/* Bloc image — hauteur fixe 50vh, image centrée */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 0.6 }}
        ...
        >
          ...
          {/* Thumbnails centrés verticalement à gauche */}
          {imageUrls.length > 1 && (
            <div className="absolute left-4 top-1/2 -translate-y-1/2 flex flex-col gap-2">
              {imageUrls.map((url, index) => (
                <button
                  key={index}
                  onClick={() => setActiveImage(index)}
                  className={`relative w-10 h-10 overflow-hidden bg-border/10 transition-opacity duration-300 ${
        ...
        {/* Bloc d'achat — accessible au scroll */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 0.6, delay: 0.1 }}
        ...
        >
          <h1 className="text-heading font-normal mb-2">{product.title}</h1>

          <p className={`text-body mb-6 ${!product.available ? 'text-muted line-through' : 'text-foreground'}`}>
            {formatPrice(product.price)}
          </p>
          ...
          {error && <p className="text-nav text-red-500 mb-4">{error}</p>}

          <div className="flex flex-col gap-2">
            <button
              onClick={handleBuyNow}
              disabled={!canBuy || isBuying}
              className={`w-full py-3 text-nav font-normal uppercase tracking-wide transition-all duration-300 ${
                canBuy ? 'bg-foreground text-background hover:opacity-80' : 'bg-border text-muted cursor-not-allowed'
              }`}
            >
              {isBuying ? '...' : product.available ? 'Acheter' : 'Épuisé'}
            </button>
            <button
              onClick={handleAddToCart}
              disabled={!canBuy}
              className={`w-full py-3 text-nav font-normal uppercase tracking-wide border transition-all duration-300 ${
                canBuy ? 'border-foreground text-foreground hover:opacity-50' : 'border-border text-muted cursor-not-allowed'
              }`}
            >
              {added ? 'Ajouté !' : 'Ajouter au panier'}
            </button>
          ...
          {product.details && (
            <p className="text-nav text-muted mt-4">{product.details}</p>
          )}
        </motion.div>
      </div>
    </div>
